...
def get_position_distribution(distribution_name, arguments, polygon_coordinates, z):
    ...
    if distribution_name == "uniform":
        return UniformPositionDistribution(arguments, polygon_coordinates, z)
    elif distribution_name == "fixed":
...
class FixedPositionDistribution:
    ...
    """Returns the given x and y coordinates"""
    def __init__(self, arguments, polygon_coordinates, z):
        assert len(arguments) == self._num_arguments, f"{self.__class__} requires {self._num_arguments} argument(s)"
...
class GaussianVelocityDistribution:
    ...
    def __init__(self, arguments):
        assert len(arguments) == self._num_arguments, f"{self.__class__} requires {self._num_arguments} argument(s)"
        self.gas_temperature, self.particle_mass, self.mean = arguments

    def get_single_velocity(self):
        sigma = math.sqrt((physics.CONSTANTS["BoltzmannConstant"] * self.gas_temperature) / self.particle_mass)
